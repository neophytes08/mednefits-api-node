...
const TransactionHelper = require(`${APPPATH}/server/helpers/transaction.helper.js`);
...
const employeeLists = async (req, res, next) => {
    ...
    for(; x < count; x++) {
        ...
        let eClaimAmountPendingMedication = await companyModel.aggregation('medi_out_of_network_transactions', [{
            $match : { 
                $and : [ 
                { claim_status: 0 }, { spending_type: 'medical' },
                 {"member_id": {
                    $in: user_ids
                }},
                ] 
        ...
        let eClaimAmountPendingWellness = await companyModel.aggregation('medi_out_of_network_transactions', [{
            $match : { 
                $and : [ 
                { claim_status: 0 }, { spending_type: 'wellness' },
                 {"member_id": {
                    $in: user_ids
                }},
                ] 
        ...
        let deletion = await companyModel.getOne("medi_customer_employee_plan_refund_details", { employee_id: member.member_id });
        let dependents = await companyModel.countCollection("medi_member_covered_dependents", { owner_id: member.member_id });
        let planTier = new Object();
        let flag = false

        let planTierUsers = await companyModel.getOne("medi_customer_plan_tier_users", { member_id: member.member_id });
        ...
        let replace = await companyModel.getOne("medi_customer_replace_employee", { member_id: member.member_id });
...
{
    let data = req.body;
    let customerId = data.customer_id ? parseInt(data.customer_id) : data.customer_id;
    ...
    // check member
    let checkEmployee = await companyModel.getOne('medi_members', { member_id: memberId });
    ...
    // check customer
    let checkCustomer = await companyModel.getOne('medi_customer_purchase', { customer_id: customerId });
    ...
    if(planned.length == 0) {
        return res.json({
           status: false,
           message: 'Customer does not have a Employee Plan Purchase.'
        });
    } else {
        planned = planned[0];
    }

    // check dependent plan status
    ...
    if(dependentPlanStatus.length == 0) {
        return res.json({
           status: false,
           message: 'Customer does not have a Dependent Plan Purchase.'
        });
    } else {
        dependentPlanStatus = dependentPlanStatus[0];
    }

    vacantSeats = parseFloat(dependentPlanStatus.dependent_head_count) - parseFloat(dependentPlanStatus.dependent_enrolled_count);
    
    // check plan tier
    let planTierUser = await companyModel.getOne('medi_customer_plan_tier_users', { member_id: memberId });
    let customerPlanTiers = null;
    let planTierStatus = false;
    let planTierId = null;

    if(planTierUser) {
        let planTier = await companyModel.getOne('medi_customer_plan_tiers', { plan_tier_id: planTierUser.plan_tier_id });
        if(planTier) {
            let total_tier_vacant = parseFloat(planTier.dependent_head_count) - parseFloat(planTier.dependent_enrolled_count);
            if((data.dependents).length > total_tier_vacant) {
                return res.json({
                    status: false,
                    message: 'We realised the current dependent headcount you wish to enroll is over the current vacant dependent seat/s of this Plan Tier.'
                })
            } else {
                planTierStatus = true;
    ...
    if((data.dependents).length > vacantSeats) {
        return res.json({
            status: false,
    ...
    let dependentPlanId = await PlanHelper.getDependentAvailableActivePlan(customerId);
    let packageGroup = await getDependentPackageGroup(dependentPlanId.dependent_plan_id);

    await map(data.dependents, async dependentElement => {
        let user = {
            name:  dependentElement.fullname,
            dob: moment(new Date(dependentElement.dob)),
            postal_code: null,
            member_type: 'dependent'
        }

        userId = await UserHelper.createUserFromDependent(user);
        ...
        if(userId)
        {
            ...
            if(familyCoverageResult)
            {
                let history = {
                    ...
                    member_id: userId, 
                    dependent_plan_id: dependentPlanId.dependent_plan_id,
                    package_group_id: packageGroup,
                    plan_start: moment(dependentElement.plan_start).format('YYYY-MM-DD'),
                    duration: '12 months',
                    type: 'started',
                    fixed: 1,
                    created_at: moment(new Date()).format("YYYY-MM-DD hh:mm:ss"),
                ...
                if(planTierId)
                {
                    await PlanTierUsersHelper.createData({
...
const getDependentPackageGroup = async (dependentPlanID) =>
{
    ...
    let secondary_account_type = null;
    ...
    if(hr)
    {
        if(parseInt(hr.wallet) == 1 && parseInt(hr.qr) == 1)
        {
            wallet = 1;
        }
    }
    ...
    if(dependentPlan.account_type == "insurance_bundle")
    {
        ...
        accountType = secondary_account_type;
    }
...
const updateCapPerVisitEmployee = async (req, res, next) =>
{
    ...
    let result = await companyModel.updateOne('medi_member_wallet', { _id: walletContainer._id }, { cap_amount: req.body.cap_amount })
...
const resendEmployeeAccount = async (req, res, next) => {
    let data = req.body;

    if(!data.member_id) {
        return res.status(400).json({ status: false, message: 'Membder ID is required.' });
    }

    data.member_id = parseInt(data.member_id);
    let member = await companyModel.getOne('medi_members', { member_id: data.member_id });
    console.log('member', member)
    if(!member) {
         return res.status(404).json({ status: false, message: 'Membder not found.' });
    }
    
    // get latest plan
     let plan_history = await companyModel.aggregation('medi_member_plan_history',
        [
            {$match: {member_id: member.member_id, type: 'started'}},
            {$sort:{created_at: -1}},
            {$limit:1}
    ...
    let password = await StringHelper.randomNumber(8);
    console.log('plan_history', plan_history);
    // update password
    let update = await companyModel.updateOne("medi_members", { _id: member._id }, { password: sha256(password) });
    let plan_start = moment(plan_history.plan_start).format("MMMM D, YYYY")
    let customer_active_plan = await companyModel.getOne('medi_customer_active_plans', { customer_active_plan_id: plan_history.customer_active_plan_id });
    let company_member = await companyModel.getOne('medi_company_members', { member_id: member.member_id });
    let company = await companyModel.getOne('medi_customer_business_information', { customer_id: company_member.customer_id });
    ...
    if(member.email) {
      var data_sent = {
        ...
        creds: member.phone_no,
        password: password,
        plan_start: plan_start,
        company: company.company_name,
        plan: customer_active_plan
      }
      // send via email
    ...
    if(member.phone_code) {
      ...
      let message = {
        body: await smsHelper.formatSmsMessage({ name: member.fullname, company: company.company_name, plan_start: plan_start, phone: member.phone_no, password: password }),
        to: phone
      }
    ...
    return res.json({ status: true, message: 'Successfully resend and reset account for Employee Member through Email/SMS.' });
}

const transferEmployee = async(req, res, next) => {
    let isValid = await employeeValidator.joiValidate({
        old_customer_id: req.body.old_customer_id,
        new_customer_id: req.body.new_customer_id,
        member_id: req.body.member_id,
        start_date: new Date(req.body.start_date),
    }, employeeValidator.transferEmployee, true);
    ...
    let oldCustomerID = parseInt(req.body.old_customer_id);
    let newCustomerID = parseInt(req.body.new_customer_id);
    let memberID = parseInt(req.body.member_id);

    let member = await companyModel.getOne("medi_members", { member_id: memberID, member_type: 'employee' });
    if(!member) {
        return res.status(404).json({
            status: false,
            message: "Member does not exist",
    ...
    let current_company = await companyModel.getOne("medi_customer_purchase", { customer_id: oldCustomerID });
    if(!current_company) {
        return res.status(404).json({
            status: false,
            message: "Current Company does not exist",
        });
    }

    let transfer_company = await companyModel.getOne("medi_customer_purchase", { customer_id: newCustomerID });
    if(!transfer_company) {
        return res.status(404).json({
            status: false,
            message: "Transfer Company does not exist",
        });
    }

    let start_date = moment(new Date(req.body.start_date)).format('YYYY-MM-DD');
    let statement_start_date = moment(start_date).startOf('month').format('YYYY-MM-DD');
    let statement_end_date = moment(start_date).endOf('month').format('YYYY-MM-DD');
    let trans_end_date = moment(new Date()).add("23", "hours").add("59 minutes").format('YYYY-MM-DD HH:MM:SS');
    var transfer_dependent_status = false;
    let current_plan = await companyModel.aggregation('medi_customer_plans',
        [
            {$match: {customer_id: oldCustomerID }},
            {$sort:{created_at: -1}},
            {$limit:1}
        ]
    );
    ...
    var dependent_accounts = await companyModel.getMany("medi_member_covered_dependents", { owner_id: memberID, deleted: 0 });

    if(dependent_accounts.length > 0) {
        let dependent_count = await companyModel.countCollection("medi_dependent_plans", { customer_plan_id: current_plan.customer_plan_id });
        
        if(dependent_count > 0) {
            let transfer_customer_plan = await companyModel.aggregation('medi_customer_plans',
                [
            ...
            transfer_customer_plan = transfer_customer_plan[0]
            let transfer_dependent_count = await companyModel.countCollection("medi_dependent_plans", { customer_plan_id: transfer_customer_plan.customer_plan_id });
            if(transfer_dependent_count == 0) {
                return res.status(400).json({ status: false, message: 'Transfer Company does not have a dependent plan purchase. Please create dependent plan purchase to enable transfer of dependent plan accounts under this current company. ' + dependent_accounts.length + ' total dependents are needed to transfer the dependent accounts' })
            }

            let transfer_data_dependent_status = await companyModel.aggregation('medi_dependent_plan_status',
                [
                    {$match: {customer_plan_id: transfer_customer_plan.customer_plan_id }},
                    {$sort:{created_at: -1}},
                    {$limit:1}
                ]
            );
            
            transfer_data_dependent_status = transfer_data_dependent_status[0];

            let dependent_vacant_seat = transfer_data_dependent_status.dependent_head_count - transfer_data_dependent_status.dependent_enrolled_count;

            if(dependent_accounts.length > dependent_vacant_seat) {
                return res.status(400).json({ status: false, message: "Employee dependent account is more than the transfer account dependent plan vacant seats. Please increase the vacant seat for transfer plan to " + (dependent_accounts.length - dependent_vacant_seat) })
            }
            transfer_dependent_status = true;
        }
    }
    
    let medi_member_plan_history = await companyModel.aggregation('medi_member_plan_history',
        [
    ...
    medi_member_plan_history = medi_member_plan_history[0]

    let current_customer_plan_status = await companyModel.aggregation('medi_customer_plan_status',
        [
            {$match: {customer_plan_id: current_plan.customer_plan_id }},
            {$sort:{created_at: -1}},
            {$limit:1}
        ]
    );
    current_customer_plan_status = current_customer_plan_status[0];
    let transfer_current_plan = await companyModel.aggregation('medi_customer_plans',
        [
            {$match: {customer_id: newCustomerID }},
            {$sort:{created_at: -1}},
            {$limit:1}
    ...
    transfer_current_plan = transfer_current_plan[0]
    let transfer_customer_plan_status = await companyModel.aggregation('medi_customer_plan_status',
        [
            {$match: {customer_plan_id: transfer_current_plan.customer_plan_id }},
            {$sort:{created_at: -1}},
            {$limit:1}
        ]
    );
    transfer_customer_plan_status = transfer_customer_plan_status[0]
    let vacant_employees = transfer_customer_plan_status.employee_head_count - transfer_customer_plan_status.employee_enrolled_count;

    if(vacant_employees <= 0) {
        return res.status(400).json({ status: false, message: 'New Company Employee Status is full. Please increase the head count of the Transfer Company.' })
    }

    // get employee transactions
    let employee_transactions = await TransactionHelper.getEmployeeTransactions(memberID, start_date, trans_end_date)
    // get new transfer company spending invoice
    // let transfer_spending_invoice = await companyModel.getMany("medi_customer_spending_invoices", { customer_id: newCustomerID, start_date: statement_start_date, end_date: statement_end_date });
    // get company active plan
    let customer_active_plan = await companyModel.getOne("medi_customer_active_plans", { customer_plan_id: transfer_customer_plan_status.customer_plan_id});

    if(medi_member_plan_history) {
        // update member plan history
        let result = await companyModel.updateMany("medi_member_plan_history", { _id: medi_member_plan_history._id}, { customer_active_plan_id: customer_active_plan.customer_active_plan_id, plan_start: start_date });
    }

    let company_member = await companyModel.getOne("medi_company_members", { member_id: memberID});
    if(company_member) {
        let result = companyModel.updateMany("medi_company_members", { _id: company_member._id}, { customer_id: newCustomerID });
    }

    // decrement current customer plan status
    let decrement_current_plan_status = await PlanHelper.employeePlanStatus(current_customer_plan_status._id, 'decrement');
    // increment transfer customer plan status
    let increment_current_plan_status = await PlanHelper.employeePlanStatus(transfer_customer_plan_status._id, 'increment');

    if(transfer_dependent_status) {
        let current_dependent_plan_status = await companyModel.getOne("medi_dependent_plan_status", { customer_plan_id: current_customer_plan_status.customer_plan_id });
        let transfer_dependent_plan_status = await companyModel.aggregation('medi_dependent_plan_status',
            [
                {$match: {customer_plan_id: transfer_customer_plan_status.customer_plan_id }},
                {$sort:{created_at: -1}},
                {$limit:1}
            ]
        );
        
        transfer_dependent_plan_status = transfer_dependent_plan_status[0];
        console.log('current_dependent_plan_status', current_dependent_plan_status);
        console.log('transfer_dependent_plan_status', transfer_dependent_plan_status);
        let dependent_plan_purchase = await companyModel.getOne("medi_dependent_plans", { customer_plan_id: transfer_customer_plan_status.customer_plan_id});
        for(var da = 0; da < dependent_accounts.length; da++) {
            let get_dependent_history = await companyModel.aggregation('medi_dependent_plan_history',
                [
                    {$match: {member_id: dependent_accounts[da].member_id, type: 'started' }},
                    {$sort:{created_at: -1}},
                    {$limit:1}
                ]
            );

            if(get_dependent_history) {
                get_dependent_history = get_dependent_history[0];
                let result = await companyModel.updateMany("medi_dependent_plan_history", { _id: get_dependent_history._id}, { dependent_plan_id: dependent_plan_purchase.dependent_plan_id });
                // decrement current dependent plan status
                let decrement_dependent_current_plan_status = await PlanHelper.dependentPlanStatus(current_dependent_plan_status._id, 'decrement');
                // increment transfer dependent plan status
                let increment_dependent_current_plan_status = await PlanHelper.dependentPlanStatus(transfer_dependent_plan_status._id, 'increment');
            }
        }
    ...
    let transaction_transfer_result = await TransactionHelper.tranfereMemberTransaction(memberID, employee_transactions, start_date, newCustomerID)
    return res.json({ status: true, message: 'Employee Account transferred successfully.' });
} 

const updateEmployeeCap = async(req, res) => {
    const data = req.body,
            employee_id = data.employee_id,
    ...
    let isValid = await employeeValidator.joiValidate(employee_parameters, employeeValidator.employeeFunc, true);
    ...
    try {
        await companyModel.updateOne('medi_member_wallet', { member_id: employee_id }, { cap_amount: cap_amount });
        res.json({
            status: true,
