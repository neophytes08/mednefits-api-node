...
const  Benefits_dashboard =  require(`${APPPATH}/server/components/company/benefits_dashboard.controller`);
    ...
    wallet = 0;
       ...
       */
       let businessInformation = {
          ...
          contact_list: {
            add_contact_first_name: data.add_contact_business_contact_first_name,
            add_contact_last_name: data.add_contact_business_contact_last_name,
            add_contact_job_title: data.add_contact_business_job_title,
            add_contact_email: data.add_contact_business_contact_email,
            add_contact_phone: data.add_contact_business_phone,
            // add_contact_hr_email: data.add_contact_business_portal_email,
            created_at: createdAt,
            updated_at: updatedAt,
            send_email_communication: (data.business_send_email_comm_related ? 1 : 0),
            send_email_billing: (data.business_send_email_bill_related ? 1 : 0)
      ...
      if(typeof customerPurchaseResult == "object")
      {
           ...
           */
           ...
           customerHRDashboard.customer_hr_account_id = await global_helper.getId('medi_customer_hr_accounts', 'customer_hr_account_id');
          /**
           ...
           */
           ...
           customerPlans.customer_plan_id = await global_helper.getId('medi_customer_plans', 'customer_plan_id');
          /**
           * Save Customer Plans
           * */
          ...
          // }
          customerActivePlans.customer_active_plan_id = await global_helper.getId('medi_customer_active_plans', 'customer_active_plan_id');
           ...
           */
           customerPlanStatus.customer_plan_status_id = await global_helper.getId('medi_customer_plan_status', 'customer_plan_status_id');
           ...
           * Active Plan Invoices
           */
           activePlanInvoices.active_plan_invoice_id = await global_helper.getId('medi_active_plan_invoices', 'active_plan_invoice_id');
           ...
           if(data.employee_plan_extension && data.employee_plan_extension == true)
           {
               ...
               */
               let activePlanExtensions = {
                ...
                activePlanExtensions.active_plan_extensions_id = await global_helper.getId('medi_active_plan_extensions', 'active_plan_extensions_id');
              /**
               * Save Active Plan Invoices
               * */
              ...
              } :
              {
               ...
               activePlanExtensionInvoice.active_plan_extension_invoices_id = await global_helper.getId('medi_active_plan_extension_invoices', 'active_plan_extension_invoices_id');
               activePlanExtensionInvoice.active_plan_extensions_id = activePlanExtensionsResult.active_plan_extensions_id;
           ...
           */
           ...
           if(data.dependent_status && data.dependent_status == true)
           {
               ...
               * Dependent Plans
               * */
               dependentPlans.dependent_plan_id = await global_helper.getId('medi_dependent_plans', 'dependent_plan_id');
               dependentPlans.customer_plan_id = customerPlansResult.customer_plan_id;
               dependentPlans.customer_id = customerID;
               dependentPlans.customer_active_plan_id = customerActivePlansResult.customer_active_plan_id;
              /**
              ...
              } :
              {
               ...
               dependentInvoices.dependent_invoice_id = await global_helper.getId('medi_dependent_invoices', 'dependent_invoice_id');
               dependentInvoices.dependent_plan_id = dependentPlansResult.dependent_plan_id;
              ...
              {
                dependentPlans = {
                  customer_plan_id: customerPlansResult.customer_plan_id,
                  customer_id: customerID,
                  customer_active_plan_id: customerActivePlansResult.customer_active_plan_id,
                  total_dependents: data.dependents_employees,
                  ...
                  } :
                  {
                   ...
                   dependentInvoices = {
                      // dependent_invoice_id: 1, //mariaDB
                      dependent_plan_id: dependentPlansResult.dependent_plan_id,
                      invoice_number: invoiceNumberFormat,
                      invoice_date: moment().format("YYYY-MM-DD"),
                      invoice_due: moment().add(1, 'months').format("YYYY-MM-DD"),
                      total_dependents: data.dependents_employees,
                      individual_price: data.dependent_plan_price_extention,
               ...
               */
               let dependentPlanStatus = {
                ...
                dependentPlanStatus.dependent_plan_status_id = await global_helper.getId('medi_dependent_plan_status', 'dependent_plan_status_id');
              /**
           ...
           * */
           let customerWallets = {
            ...
            customerWallets.customer_wallet_id = await global_helper.getId('medi_customer_wallets', 'customer_wallet_id');
          /**
           * Save Customer Wallets
           * */
           ...
           */
           if(parseFloat(data.medical_spending_credits) > 0)
           {
            let customerMedicalHistory = {
                  ...
                  customer_id: customerID,
                  customer_wallet_id: customerWalletsResult.customer_wallet_id,
                ...
                customerMedicalHistory.customer_wallet_history_id = await global_helper.getId('medi_customer_wallet_history', 'customer_wallet_history_id');
              /**
               * Save Customer Wallets
               * */
           ...
           */
           if(parseFloat(data.wellness_spending_credits) > 0)
           {
            let customerWellnessHistory = {
                ...
                customerWellnessHistory.customer_wallet_history_id = await global_helper.getId('medi_customer_wallet_history', 'customer_wallet_history_id');
              /**
               * Save Customer Wallets
               * */
          ...
          if(parseFloat(data.deposit_medical) > 0 || parseFloat(data.deposit_wellness) > 0)
          {
            ...
            let customerSpendingDepositCredits = {
                ...
                customerSpendingDepositCredits.customer_spending_deposit_credit_id = await global_helper.getId('medi_customer_spending_deposit_credits', 'customer_spending_deposit_credit_id');
              /**
               * Save Customer Wallets
               * */
          ...
          if(data.send_email)
          {
            ...
            if(data.employee_payment_status == false)
            {
              ...
              let results = await PlanHelper.planInvoiceDetails(activePlanInvoicesResult.active_plan_invoice_id);
              if(results) {
                let data = {
...
const updatePlanTierEmployeeEnrollment = async ( req, res, next ) =>
{
  ...
  let data = req.body;
  let temp_enrollment_id = data.temp_enrollment_id
  let updatedAt = moment().format("YYYY-MM-DD HH:mm:ss");
  ...
  //     return res.json({
  //         status: false,
  //         message: "Employee Enrollee Job Title is required."
  //     });
  // }
  ...
  let dataUpdates = {
    ...
    let dataUpdatesResult = await companyModel.updateMany("medi_customer_employee_temp_enrollment", { _id: temp_enrollment_id}, dataUpdates);
...
const enrollmentEmployeeValidation = async (user, exceptEnrolleEmailValidation, customerID) => {
  ...
  // isValid = await validate.joiValidate(user.last_name, validate.addEmployee.enrollmentEmployeeValidation.last_name, true)

  // if(typeof isValid != 'boolean')
  // {
  //     empValidation.last_name_error = true;
  //     empValidation.last_name_message = `*${isValid.details[0].message}`;
  // }
  // else
  // {
  //     empValidation.last_name_error = false;
  //     empValidation.last_name_message = '';
  // }
  ...
  //     let memberNRIC = await companyModel.getOne('medi_members', {nric: user.nric});

  //     if(checkDuplicateNRIC || memberNRIC)
  //     {
  //         empValidation.nric_error = true;
  //         empValidation.nric_message = '*NRIC/FIN is must be valid';
  //     }
  //     else
  //     {
...
const createEmployee = async (enrollmentID, customerID) => {
  ...
  corporateMember.member_id = userID;
  corporateMember.company_member_id = await global_helper.getId('medi_company_members', 'company_member_id');
  ...
  await enrollDependents(enrollmentID, customerID, userID, planned.customer_plan_id)
  let user_data = await companyModel.getOne('medi_members', { member_id: userID });
...
const creaditsAllocation = async (customerID, dataEnrollee, customerActivePlanID, userID, activePlan, createdAt, updatedAt, walletType, customer) => {
  ...
  let walletHistory = {
    "wallet_history_id": await global_helper.getId('medi_member_wallet_history', 'wallet_history_id'),
    ...
    let customerCreditsResult = await CustomerCreditsHelper.deductCustomerCredits(customer.customer_wallet_id, (walletType == "medical" ? dataEnrollee.medical_credits : dataEnrollee.wellness_credits), (walletType == "medical" ? "medical" : "wellness") );
  ...
  // {
    ...
    let companyDeductLogs = {
      customer_wallet_history_id: await global_helper.getId('medi_customer_wallet_history', 'customer_wallet_history_id'),
...
const enrollDependents = async (tempEnrollmentID, customerID, employeeID, customerPlanID) =>
{
  ...
  if(dependentEnrollees)
  {
    ...
    await map(dependentEnrollees, async element => {
      ...
      {
        groupPackageID = packageGroup.benefits_package_group_id
      }
      ...
      if(userID)
      {
        ...
        {
                  // create dependent plan history
                  let history = {
                    dependent_plan_history_id: await global_helper.getId('medi_dependent_plan_history', 'dependent_plan_history_id'),
                    member_id: userID,
                    dependent_plan_id: element.dependent_plan_id,
                    package_group_id: groupPackageID,
                    plan_start: moment(element.plan_start).format('YYYY-MM-DD'),
                    duration: '12 months',
                    type: 'started',
                    fixed: 1,
                    created_at: moment(new Date()).format("YYYY-MM-DD hh:mm:ss"),
                  ...
                  companyModel.saveOne('medi_dependent_plan_history', history);
...
const getCompanyTotalCreditAllocation = async (req, res, next) => {
...
const allocateCompanyCredits = async(req, res, next) => {
  if(!req.body.customer_id) {
    return res.status(400).json({ status: true, message: 'Customer ID is required.' });
  }
  ...
  if(spendingType == "medical" || spendingType == "wellness") {
    if(validation.allocation_type == "add") {
      // add credits
      let walletResult = await companyModel.updateOne("medi_customer_wallets",
          {"customer_id":  customerID},
          (spendingType == "medical" ? {$inc:{medical_balance: (parseFloat(validation.credits))}, updated_at: date_now} : {$inc:{wellness_balance: (parseFloat(validation.credits))}, updated_at: date_now})
          );
      ...
      if(walletResult) {
        ...
        let wallet_logs = {
          ...
          customer_id: customerID,
          credit: parseFloat(validation.credits),
          running_balance: spendingType == "medical" ? companyCredits.medical_balance + validation.credits : companyCredits.wellness_balance + validation.credits,
          type: 'admin_added_credits',
          customer_active_plan_id: data.customer_active_plan_id ? data.customer_active_plan_id : null,
        ...
        // save logs
        companyModel.saveOne('medi_customer_wallet_history', wallet_logs);
        return res.status(201).json({ status: true, message: `Company added ${ validation.credits } credits for ${ spendingType } wallet.` })
      } else {
        return res.status(400).json({ status: false, message: 'Unable to add credits to the company. Please try again later.' });
      }
    } else if(validation.allocation_type == "deduct") {
      // deduct credits
      let walletResult = await companyModel.updateOne("medi_customer_wallets",
          {"customer_id":  customerID},
          (spendingType == "medical" ? {$inc:{medical_balance: (parseFloat(validation.credits) * -1)}, updated_at: date_now} : {$inc:{wellness_balance: (parseFloat(validation.credits)) * -1}, updated_at: date_now})
          );
      ...
      if(walletResult) {
        ...
        // save logs
        companyModel.saveOne('medi_customer_wallet_history', wallet_logs);
        return res.status(201).json({ status: true, message: `Company deducted ${ validation.credits } credits for ${ spendingType } wallet.` })
      } else {
        return res.status(400).json({ status: false, message: 'Unable to add credits to the company. Please try again later.' });
      }
    } else {
      return res.status(400).json({
        status: false,
        message: 'Please choose a allocation type ["add", "deduct"]'
      });
    }
  } else {
    return res.status(400).json({
  ...
  return res.json(companyCredits);
}
...
let corporate = async (req, res) => {
  const method = req.method,
        isGetCSV = req.query.isGetCSV;
  switch (method) {
    case "GET":
      if (isGetCSV) {
        return exportCorprateList(req, res);
      } else {
        return corporateList(req, res);
      }
    case "PUT":
    case "POST":
    case "DELETE":
    default:
      return res.status(404).json({
        status: false,
        message: "Method used invalid",
      });
  }
};

// GET LIST OF CORPORATE
let corporateList = async (req, res) => {
      try {
        const data = req.query;
        let corporateData = [],
            skip = data.page ? data.page : 0,
            limit = data.limit ? data.limit : 10;
        let params = {}, customer_plan_params = {};

        // List of Corporate
        if (data.search) {
          params = { 'company_name': { $regex: data.search, $options: 'i' }};
        } else {
          params = {};
        }     
        let corporate = await companyModel.getLimit('medi_customer_business_information', params, limit, skip);
                    
        // List of Employee per corporate 
        for (let i =0 ; i < corporate.length ; i++) {
          // Create parameter for customer plan
            if (data.start && data.end) {
              customer_plan_params = { 
                plan_end: {"$gte": data.start, "$lt": data.end},
                customer_id: corporate[i].customer_id
              };
            }

          let customer_plan = await companyModel.getOne('medi_customer_plans', customer_plan_params);
          
          // return null if query does not return value even there are corporate value. 
          if (!customer_plan) {
            corporateData = [];
            break;
          }

          let plan_status = await companyModel.getOne('medi_customer_plan_status', { customer_plan_id: customer_plan.customer_plan_id });
          let hr = await companyModel.getOne('medi_customer_hr_accounts', { customer_id: corporate[i].customer_id });
          let customer_dependent_plan_status = await companyModel.getOne('medi_dependent_plan_status', { customer_plan_id: plan_status['customer_plan_id'], customer_id: corporate[i].customer_id });
          let credits = await getCredits(customer_plan);

            corporateData.push({
              corporate : corporate[i],
              employees : plan_status,
              hr  : hr,
              total_employee_seat: plan_status['employee_head_count'],
              total_dependent_seat: customer_dependent_plan_status ? customer_dependent_plan_status['dependent_head_count'] : null,
              expiry_date : customer_plan['plan_end'],
              total_medical_credits: credits['total_medical_company_allocation']
            });

        }
      
        // Send data.
        let lastPage = corporateData.length >= parseInt(limit) ? (corporateData.length /parseInt(limit)) : corporateData.length;
          
        return res.json({
            data: corporateData,
            status: true,
            current_page: data.page ? data.page : 1,
            last_page: lastPage,
            total: corporateData.length,
            per_page: limit,
            last_item_page: corporateData.length >= parseInt(limit) ? (parseInt(parseInt(limit)) * (data.page ? data.page : 1)) : corporateData.length,
        });
...
// Export CSV
let exportCorprateList = async (req, res) => {
  const data = req.query,
        corporate_ids = data.ids,
  ...
  const isValid = await validate.joiValidate(query_params,validate.exportCorporateCSV, true);
  

  // Check if those valid parameters if falsy
  if (typeof isValid !== 'boolean' ) {
    return errorFunc(res,{
      status: false,
      message: isValid.details[0].message
  ...
  try {
      // Contains corporate
      let corporate_params = [
        {
          model: 'medi_customer_business_information',
          ref: 'customer_id',
          fields: { company_name: true, customer_id: true },
          params: by_id ? { customer_id: { $in: corporate_ids }} : {}
        },
        {
          model: 'medi_company_members',
          ref: 'member_id',
          fields: { member_id: true , customer_id: true },
          params: by_id ? { customer_id: { $in: corporate_ids }} : {}
        }
      ];
      
      let coprate_details = await companyModel.getTwoTable(corporate_params);
      
      for (let x = 0; x < coprate_details.length; x++) {
        // contains customer plans
        
        let customer_plan_params = {};
        // Verify filter by date
        if (by_date) {
          customer_plan_params = { 
            plan_end: {"$gte": start_date, "$lt": end_date},
            customer_id: coprate_details[x].customer_id
          };
        } else {
          customer_plan_params = { 
            customer_id: coprate_details[x].customer_id
        ...
        let corporate_plans = await companyModel.aggregation('medi_customer_plans', 
        [
            {$match: customer_plan_params },
            {$sort:{created_at: 1}},
            {$limit:1}
        ]);

        // Condition for filtering with plan_start and plan_end
        if (corporate_plans.length > 0) {
          
            // Corporate plan statuses
            let corporate_plan_statuses = await companyModel.getOne('medi_customer_plan_status', {});
            
            // corporate dependent statuses
            let corporate_dependent_statuses = await companyModel.aggregation('medi_dependent_plan_status', 
            [
                {$match: {
                  customer_plan_id: corporate_plans[0]['customer_plan_id']
                } },
                {$sort:{created_at: 1}},
                {$limit:1}
            ]);
            ...
            let corporate_active_plan = await companyModel.getMany('medi_customer_active_plans', 
            { 
              customer_id : corporate_plans[0]['customer_id'],
              customer_plan_id: corporate_plans[0]['customer_plan_id']
            });

            // Calcualte no.of employees  * individual_price
            let plan_amount = 0;

            for (let y = 0; y < corporate_active_plan.length; y++) {
              // Corporate Active invoices
              let corporate_active_invoices = await companyModel.getOne('medi_active_plan_invoices', { 
                customer_id: corporate_active_plan[y]['customer_id'],
                customer_active_plan_id: corporate_active_plan[y]['customer_active_plan_id']
              });

              // Start computation
              plan_amount += parseInt(corporate_active_invoices['customer_active_plan_id']) * parseFloat(corporate_active_invoices['individual_price']);
            }

            // Corporate HR accounts
            let corporate_hr_accounts = await companyModel.getOne('medi_customer_hr_accounts', { customer_id: coprate_details[x]['customer_id'] });
            
            // Corporate member wallets
            let corporate_member_wallets = await companyModel.getOne('medi_member_wallets', { member_id: coprate_details[x]['member_id'] });
            
            // Corporate Member Details
            let corporate_member_details = await companyModel.getOne('medi_members', { member_id: coprate_details[x]['member_id'] });
          
            // Check column if Employee Fullname is available.
            const withEmployeeFullname = _.includes(dataIndex, 'employee_fullname');
            
            corporateData.push({
              plan_start: withEmployeeFullname ? ( x <= 0 ? corporate_plans[0]['plan_start']: ''): corporate_plans[0]['plan_start'],
              plan_end: withEmployeeFullname ? (x <= 0 ? corporate_plans[0]['plan_end']: ''): corporate_plans[0]['plan_end'],
              account_type: withEmployeeFullname ? (x <= 0 ? corporate_plans[0]['account_type']: '') : corporate_plans[0]['account_type'],
              company_name: coprate_details[x]['company_name'] ,
              no_employees : withEmployeeFullname ? (x <= 0 ? corporate_plan_statuses['employee_enrolled_count']: '') : corporate_plan_statuses['employee_enrolled_count'],
              total_employee_seats : withEmployeeFullname ? (x <= 0 ? corporate_plan_statuses['employee_head_count']: ''): corporate_plan_statuses['employee_head_count'],
              total_dependent_seat: withEmployeeFullname ? (x <= 0 ? (corporate_dependent_statuses.length > 0 ? corporate_dependent_statuses[0]['dependent_head_count'] :''): ''): corporate_dependent_statuses.length > 0 ? corporate_dependent_statuses[0]['dependent_head_count'] :'',
              no_dependents: withEmployeeFullname ? (corporate_dependent_statuses.length > 0 ? corporate_dependent_statuses[0]['dependent_enrolled_count']: ''): corporate_dependent_statuses.length > 0 ? corporate_dependent_statuses[0]['dependent_enrolled_count']: '',
              plan_amount: withEmployeeFullname ? (x <= 0 ? plan_amount: ''): plan_amount,
              hr_email: withEmployeeFullname ? (x <= 0 ? corporate_hr_accounts['email']: ''): corporate_hr_accounts['email'],
              hr_account_status: withEmployeeFullname ? (x <= 0 ? (corporate_hr_accounts['active'] ? 'Active': 'InActive'): ''): corporate_hr_accounts['active'] ? 'Active': 'InActive',
              total_medical_credit: withEmployeeFullname ? (x <= 0 ? getCredits(coprate_details[x])['total_medical_company_allocation']: ''): getCredits(coprate_details[x])['total_medical_company_allocation'],
              wellness_credit: withEmployeeFullname ? (x <= 0 ? getCredits(coprate_details[x])['total_medical_wellness_allocation']: ''): getCredits(coprate_details[x])['total_medical_wellness_allocation'],
              medical_credits: withEmployeeFullname ? (x <= 0 ? getCredits(coprate_details[x])['total_medical_employee_allocated']: ''): getCredits(coprate_details[x])['total_medical_employee_allocated'],
              medical_spent_credits: withEmployeeFullname ? (x <= 0 ? getCredits(coprate_details[x])['total_medical_employee_spent']: ''): getCredits(coprate_details[x])['total_medical_employee_spent'],
              wellness_spent_credits: withEmployeeFullname ? (x <= 0 ? getCredits(coprate_details[x])['total_wellness_employee_spent']: ''): getCredits(coprate_details[x])['total_wellness_employee_spent'],
              cap_status: corporate_member_wallets['cap_amount'] > 0 ? 'With Cap': 'No Cap',
              employee_email: corporate_member_details['email'],
              employee_dob: corporate_member_details['dob'],
              employee_status: corporate_member_details['active'] ? 'Active' : 'InActive',
              employee_fullname: corporate_member_details['fullname'],
              employee_phone_no: corporate_member_details['phone_no']  
            });          
        }

      }
      console.log(corporateData)
    // Get column selected value.
    if (corporateData.length > 0) {
    
      res.status(200);
      res.setHeader('Content-disposition', 'attachment; filename=Corporate List.xlsx');
    ...
    var worksheet = workbook.addWorksheet('Corporate List Sheet')
     // Create Header
     worksheet.addRow(column_header).commit();
    
     _.forEach(corporateData, data => {
      let xls_row_data = [];

      _.forEach(dataIndex, column => {
          if (typeof data[column.trim()] !== 'undefined') {
            xls_row_data.push(data[column.trim()]);
          } else  {
            xls_row_data.push('');
          }
      });
      ...
      // Create Row Data
        worksheet.addRow(xls_row_data).commit();
    });
...
let getCredits = async (data) => {

  let customerID = parseInt(data.customer_id);
  let totalAllocationWellness = null;
  let deletedEmployeeAllocationWellness = 0;
  let checkAccessibility = await Benefits_dashboard.hrStatus(customerID);
  ...
  if(checkAccessibility.accessibility == 1)
  {
      ...
      totalAllocationMedical = await Benefits_dashboard._checkBalanceMedicalWellness(customerCreditResetMedical, "medical", customerID);
      let customerCreditResetWellness = await companyModel.aggregation('medi_wallet_resets',
          [
              {$match: {
                  id: customerID,
      ...
      totalAllocationWellness = await Benefits_dashboard._checkBalanceMedicalWellness(customerCreditResetWellness, "wellness", customerID);
...
let getCorporateDetails = async (req, res, next) => {
  let data = req.query;

  if(!data.customer_id) {
    return res.status(404).json({ status: false, message: 'customer_id is required' });
  }
  data.customer_id = parseInt(data.customer_id);
  let customer = await companyModel.getOne('medi_customer_purchase', { customer_id: data.customer_id});

  if(!customer) {
    return res.status(404).json({ status: false, message: 'Customer does not exist' });
  }
  let customer_plan = await companyModel.aggregation('medi_customer_plans',
      [
          {$match: {customer_id: data.customer_id }},
          {$sort:{created_at: -1}},
          {$limit:1}
      ]
  );
  customer_plan = customer_plan[0];
  // get company employee plan status
  let employee_plan_status = await companyModel.getOne('medi_customer_plan_status', { customer_plan_id: customer_plan.customer_plan_id});
  // get company dependent plan status
  let dependent_plan_status = await companyModel.getOne('medi_dependent_plan_status', { customer_plan_id: customer_plan.customer_plan_id});
  // get company business inforamation
  let info = await companyModel.getOne('medi_customer_business_information', { customer_id: customer.customer_id});
  // get company plan subscription plan status
  let plan_status = await PlanHelper.getPlanExpiration(data.customer_id, res);

  // get company credit reset
  let reset_credit = await companyModel.aggregation('medi_wallet_resets',
      [
          {$match: {id: data.customer_id, user_type: 'company' }},
          {$sort:{created_at: -1}},
          {$limit:1}
      ]
  );
  ...
  let information = {
    ...
    plan_start: plan_status.start_date,
    plan_end: plan_status.end_date,
    days_left: plan_status.days_left,
    to_expire: plan_status.expire,
    plan_renewal: plan_status.expire,
    spending_account_renewal: plan_status.spending_account_renewal,
    combine: plan_status.combine,
    last_credit_reset_data: reset_credit.length > 0 ? reset_credit[0].date_resetted : null
  }
  return res.json(information);
}

let getCorproateCredits = async (req, res, next) => {
  let data = req.query;
  ...
  let credits = await getCredits({ customer_id: data.customer_id });
...
let getCompanyPlanRenewalStatus = async (req, res, next) => {
  if(!req.query.customer_id) {
    return res.status(400).json({ status: true, message: 'Customer ID is required.' });
  }
  ...
  let plan_status = await PlanHelper.getPlanExpiration(customerID, res);
  res.json({ status: true, to_plan_renew: plan_status.to_expire, to_spending_renew: plan_status.to_spending_renew, combine: plan_status.combine });
}
...
const getEmployeeDetails = async (req, res, next) => {
  ...
  try {
    ...
    let memberExist = await companyModel.getOne("medi_members", { member_id: memberID });
    ...
    let memberDetails = await companyModel.getMany("medi_members", { member_id: memberID });
    let memberEntitlement = await companyModel.getMany("medi_employee_wallet_entitlements", { member_id: memberID });

    let info = {
      fullname: memberDetails.fullname,
      nric: memberDetails.nric,
      date_of_birth: memberDetails.dob,
      postal_code: memberDetails.postal_code,
      job_title: memberDetails.job_title,
      mobile_country_code: memberDetails.phone_code,
      mobile_number: memberDetails.phone_no,
      medical_entitlement: memberEntitlement.medical_entitlement,
      member_id: memberDetails.member_id,
      work_email: memberDetails.email,
      bank_account_number: memberDetails.bank_account_number,
      bank_code: memberDetails.bank_code,
      bank_brh: memberDetails.bank_brh,
      for_communication: memberDetails.communication_type,
      mobile_number_back_up: null,
      wellness_entitlement: memberEntitlement.wellness_entitlement,
    }
...
module.exports = {
  ...
  getCompanyTotalCreditAllocation,
  getCompanyCompanyInformation,
  updateCompanyBusinessContact,
  updateCompanyBillingContact,
  getDependentPackageGroup,
  PackagePlanGroup,
  FamilyCoverageAccounts,
  allocateCompanyCredits,
  testEmail,
  corporate,
  getCorporateDetails,
  getCorproateCredits,
  getCompanyPlanRenewalStatus,
  getEmployeeDetails
};
