  ...
  fs.readFile(schemaSourceModels, async (err, data) => {  
      ...
      modelList = new Promise(async function(resolve, reject) {
        await _.each(schemas, async (value, key) => {
          Schema = new mongoose.Schema(value, { timestamps: true });
          Schema.index({'$**': 'text'});
...
module.exports = {
  ...
  getPrimaryID: async (model, id, objectContainer) => {
    let primaryKeys = {
      [model]: id
    }
          ...
          // return 0
            ...
            if(typeof mongoResult[0] != 'undefined')
            {
              if(!isNaN(mongoResult[0].maxInfoID))
              {
                ...
                return mongoResult[0].maxInfoID + 1;
              }
            }
  ...
  getPrimaryIDOld: async (model, objectContainer) => {
    console.warn('test')
    let primaryKeys = {
      medi_admin: "_id"
