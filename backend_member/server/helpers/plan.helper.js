...
const getEnrolleePackages = async (customerActivePlanID, planAddOn) => {
    ...
    console.log('planAddOn', planAddOn)
    ...
    if(activePlan)
    {
        ...
        {
            let trial = await mongoose.fetchOne("medi_benefits_package_group", { default_selection: 1 });
        }
    }
    ...
    if(packageGroup)
    {
        return packageGroup.benefits_package_group_id;
    }
...
const getCompanyAvailableActivePlan = async (customerID) => {
    ...
    let check = await mongoose.fetchOne("medi_customer_purchase", {_id: customerID});
    ...
    let activePlan = await mongoose.fetchMany("medi_customer_active_plans", {plan_id: plan[0].customer_plan_id});
...
const memberAllocatedCredits = async (memberWalletID, memberID, walletType) => {
    try {
        ...
        return {
            ...
            inNetworkSpent: getAllocationSpentTemp,
            eClaimSpent: eClaimSpent,
...
const memberActivityAllocatedCredits = async (memberWalletID, memberID, walletType, start) => {
    try {
        var getAllocation = 0;
        var deductedCredits = 0;
        var creditsBack = 0;
...
const getEmployeePlanType = async (customerActivePlanID) => {
    let activePlan = await mongoose.fetchOne("medi_customer_active_plans", {customer_active_plan_id: customerActivePlanID});
    let plan_name = "Mednefits Care Plan (Corporate)";
    ...
    if(activePlan.account_type == "insurance_bundle") {
        if(activePlan.secondary_account_type == null) {
            if(activePlan.secondary_account_type == "pro_plan_bundle"){
                plan_name = "Mednefits Care: Bundle Pro";
            } else {
                plan_name = "Mednefits Care: Bundle Lite";
            }
        } else if(activePlan.secondary_account_type == "pro_plan_bundle"){
            plan_name = "Mednefits Care: Bundle Pro";
        } else {
            plan_name = "Mednefits Care: Bundle Lite";
        }
    } else if(activePlan.account_type == "stand_alone_plan") {
...
const planInvoiceDetails = async (invoice_id) => {
    ...
    let invoice = await mongoose.fetchOne('medi_active_plan_invoices', { _id: invoice_id })
    console.log('invoice', invoice)
    if(!invoice) {
        return false;
    }

    let active_plan = await mongoose.fetchOne('medi_customer_active_plans', { _id: invoice.customer_active_plan_id })
    ...
    if(active_plan.new_head_count == 0) {
        ...
        let firstPlan = await mongoose.fetchOne("medi_customer_active_plans", { _id: invoice.customer_active_plan_id })
        plan = await mongoose.fetchOne("medi_customer_plans", { _id: firstPlan.customer_plan_id })
...
const getEmployeePlanDetails = async (member_id) => {
    let deletion = await mongoose.fetchOne("medi_customer_employee_plan_refund_details", { employee_id: member_id });
    let getEmployeePlan = await mongoose.aggregation('medi_member_plan_history',
        [
            {$match: {member_id: member_id, type: 'started'}},
            {$sort:{created_at: -1}},
            {$limit: 1}
        ]
    );
    ...
    let replace = await mongoose.fetchOne("medi_customer_replace_employee", { member_id: member_id });
    let activePlan = await mongoose.fetchOne("medi_customer_active_plans", { customer_active_plan_id: getEmployeePlan.customer_active_plan_id });
    let expiryDate = null;
    ...
    {
        replace = replace;
        expiryDate =replace.expired_and_activate;
    } else {
        plan = await mongoose.aggregation('medi_customer_plans',
            [
        ...
        let activePlanFirst = await mongoose.fetchOne("medi_customer_active_plans", { customer_plan_id: plan.customer_plan_id });
        
        if(parseInt(activePlanFirst.plan_extension_enable) == 1)
        {  
            let activePlanExtension = await mongoose.fetchOne('medi_active_plan_extensions', { customer_active_plan_id: activePlanFirst.customer_active_plan_id });
    ...
    if(deletion)
    {
        expiryDate = deletion.date_withdraw;
    }

    // get employee valid claim
    let valid_claim_date = await mongoose.fetchOne('medi_member_plan_history', { member_id: member_id });
    console.log('valid_claim_date', valid_claim_date)
    return { plan_start: moment(getEmployeePlan.plan_start).format('YYYY-MM-DD'), plan_end: moment(expiryDate).format('YYYY-MM-DD'), valid_start_claim: moment(valid_claim_date.plan_start).format('YYYY-MM-DD'), valid_end_claim: moment(expiryDate).format('YYYY-MM-DD')}
}
module.exports = {
    ...
    planInvoiceDetails,
    getEmployeePlanType,
    getEmployeePlanDetails,
    memberActivityAllocatedCredits
}