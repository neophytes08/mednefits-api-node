...
const addTempEmployee = async (req, res, next) => {
    ...
    // get the latest id
    let temp_id = await companyModel.aggregation('medi_customer_employee_temp_enrollment',
        [
            {$sort: {temp_enrollment_id: -1}},
            {$limit: 1}
        ]
    );
    ...
    try {
        ...
        await map(data.employees, async element => {
            ...
            tempEnrollment = {
                ...
                'mobile_area_code': element['mobile_area_code'],
            ...
            tempEnrollment._id = await global_helper.createUuID();
                ...
                if(typeof element.dependents != "undefined")
                {
                    if((element.dependents).length > 0)
                    {
                        ...
                        await map(element.dependents, async dependent => {
                            dependent.plan_start = moment(new Date()).format("YYYY-MM-DD");

                            dependentPlanID = await getCompanyAvailableDependenPlanId(dependent);

                            console.warn(customerActivePlan)
                            if(!dependentPlanID)
                            {
                            ...
                            let enrollmentDependentResult = await companyModel.saveOne("medi_dependent_temp_enrollment",
                                await companyModel.getPrimaryID(
                                    "medi_dependent_temp_enrollment",
                                    {
                                        'employee_temp_id': enrollmentDataResult._id,
                                        'dependent_plan_id': dependentPlanID,
                                        'plan_tier_id': planTierID,
                                        'first_name': dependent['first_name'],
                                        'last_name': dependent['last_name'],
                                        'nric': dependent['nric'],
                                        'dob': moment(new Date(dependent['dob'])).format("YYYY-MM-DD"),
                                        'plan_start': moment(new Date(dependent['plan_start'])).format("YYYY-MM-DD"),
...
const enrollmentDepedentValidation = async (dependent, customerID) => {
    ...
    isValid = await validate.joiValidate(dependent.first_name, validate.addEmployee.employeeDependentValidation.first_name, true)
    
    if(typeof isValid != 'boolean' || !isValid)
    {
        empValidation.first_name_error = true;
        empValidation.first_name_message = `*${isValid.details[0].message}`;
    }
    else 
    {
        empValidation.first_name_error = false;
        empValidation.first_name_message = '';
    }

    isValid = await validate.joiValidate(dependent.last_name, validate.addEmployee.employeeDependentValidation.last_name, true)
    ...
    isValid = await validate.joiValidate(dependent.nric, validate.addEmployee.employeeDependentValidation.nric, true)
...
const getCompanyAvailableDependenPlanId = async(customerID) =>
{
    ...
    let plan = await companyModel.aggregation('medi_customer_plans',
        [
            {$match: {customer_plan_id: customerID}},
            {$sort:{ created_at: -1}},
            {$limit: 1}
        ]
    );
    ...
    await map(dependentPlans, async dependent => {
        ...
        pendingEnrollment = parseFloat(dependent.total_dependents) - activeUsersCount;
...
const getCompanyPlanDates = async (customerID) =>
{
    ...
    let activePlan = await companyModel.getOne('medi_customer_active_plans', {customer_id: customerID});
...
const enrollmentEmployeeValidation = async (user, exceptEnrolleEmailValidation, customerID) => {
    ...
    if(!user.mobile)
    {
        ...
        isValid = await validate.joiValidate(user.email, validate.addEmployee.enrollmentEmployeeValidation.email, true)
        ...
        {
            empValidation.email_error = false;
            empValidation.email_message = '';
        }
    ...
    isValid = await validate.joiValidate(user.first_name, validate.addEmployee.enrollmentEmployeeValidation.first_name, true)
...
{
    let data = req.body;
    let enrollmentID = data.temp_enrollment_id;
    ...
    let checkEnrollee = await companyModel.getOne("medi_customer_employee_temp_enrollment", {_id: enrollmentID})
...
const removeEnrollee = async (req, res, next) =>
{
    ...
    let id = req.query.id;
    let result = await companyModel.removeData("medi_customer_employee_temp_enrollment", {_id: id});
    if(result)
    {
...
const createEmployee = async (enrollmentID, customerID) => {
    ...
    let dataEnrollee = await companyModel.getOne("medi_customer_employee_temp_enrollment",{_id: enrollmentID});
    ...
    {
        activePlan = await companyModel.getOne("medi_customer_active_plans", {_id: await customerActivePlanID});
        // console.warn('here')
        // console.warn(activePlan)
    }

    let corporate = await companyModel.getOne("medi_customer_purchase", {_id: customerID});
    ...
    let password = sha256("mednefits");
    
    let data = {
        name: `${dataEnrollee.first_name} ${dataEnrollee.last_name}`,
        password: password,
    ...
    corporateMember._id = await global_helper.createUuID();
    ...
    let customerPlanAdjustmentResult = await CustomerPlanStatus.addjustCustomerStatus('employee_enrolled_count', planned._id, 'increment', 1);
...
{
    let dependentEnrollees = companyModel.getMany(
        "medi_dependent_temp_enrollment",
        {employee_temp_id: tempEnrollmentID}
    );

    if((Object.keys(dependentEnrollees || {})).length > 0)
    {
        let data = null;
        ...
        await map(dependentEnrollees, async element => {
            data = {
                name: `${element.first_name} ${elementlast_name}`,
                email: 'mednefits',
                phone_code: '+65',
                nric: element.nric,
                active: 1,
            ...
            if((Object.keys(packageGroup || {})).length <= 0)
            {
            ...
            if(userID)
            {
                let familyCoverageResult = await FamilyCoverageAccounts.createData({
                    member_covered_dependent_id: 1,//primary
                    owner_id: employeeID,
                    member_id: userID,
                ...
                if((Object.keys(familyCoverageResult || {})).length > 0)
                {
                    if(element.plan_tier_id)
                    {
                        let planTierHistory = {
                            plan_tier_user_id: 1,//primaryID
