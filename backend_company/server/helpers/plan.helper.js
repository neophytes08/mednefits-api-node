...
const getCompanyAccountTypeEnrollee = async (customerID) => {
    ...
    let hr = await mongoose.fetchOne('medi_customer_purchase', {_id: customerID});
...
const getEnrolleePackages = async (customerActivePlanID, planAddOn) => {
    ...
    let activePlan = await mongoose.fetchOne("medi_customer_active_plans", {_id: customerActivePlanID});
    ...
    if(activePlan)
    {
        ...
        {
            let plan = await mongoose.fetchOne("medi_customer_plans", {_id:  activePlan.customer_plan_id})
