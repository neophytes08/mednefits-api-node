...
const getEmployeeDependents = async(req, res, next) =>
{
    ...
    if(dependents.length > 0)
    {
        ...
        for(var x = 0; x < dependents.length; x++) {
            let member = dependents[x];
            console.log('member', member);
            var mediUsers = await companyModel.getOne('medi_members', { member_id: member.member_id, member_type: "dependent", active: 1 });
            if(mediUsers) {
                ...
                deletion = false;
                deletionText = null;
                withdraw = await companyModel.getOne('medi_dependent_plan_withdraws', { member_id: member.member_id })
                ...
                replace = await companyModel.getOne('medi_customer_replace_dependent', { member_id: member.member_id })
                ...
                temp.relationship = (member.relationship ? member.relationship : 'Dependent');
                ...
                temp.fullname = mediUsers.fullname;
...
const updateEmployeeDetails = async(req, res, next) =>
{
    ...
    try {
        ...
        if(!member) {
            return res.status(404).json({ status: false, message: 'yowyow' })
        }
...
const updateDependentDetails = async (req, res, next) =>
{
    ...
    try {
        ...
        if(!member) {
            return res.status(404).json({ status: false, message: 'Member does not exist.' })
        }
...
const inNetworkOutNetworkFullTransactionList = async (req, res, next) =>
{
  let memberID = null;
  let data = req.body;
  let data_info = new Object();
  let inNetwork = await companyModel.getMany("medi_member_in_network_transactions");
  let outNetwork = await companyModel.getMany("medi_out_of_network_transactions");

  let data_temp = {
    In_Network: {
      Full_transactions: inNetwork,
    },
    Out_Network: {
      Full_transactions: outNetwork,
    },
  }
...
module.exports = {
    ...
    inNetworkOutNetworkFullTransactionList
}
